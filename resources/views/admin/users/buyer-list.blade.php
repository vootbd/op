...
@push('custom-style')
<link rel="stylesheet" href="{{ asset('css/admin/css/buyer-list.css') }}">
...
<li class="breadcrumb-item active" aria-current="page">バイヤ一覧 </li>
@endpush
        ...
        columns: [
            ...
            },
            {
                ...
                'render': function (data,type,row) { 
                    return "<div class = 'vendor-first'><span style='display:block;'><a href='/users/"+row.id+"/edit' class='link'>"+data+"</a></span><div class = 'pr-sm-0 pr-md-5 pt-sm-3'  style = 'display:inline-block; padding-top:5px!important;'><span><a href='/users/"+row.id+"/edit'><img class='icon-support pr-2' style = 'height:25px' src='{{ asset('image/message.png') }}'' alt='island icon'></a></span><a href='/users/"+row.id+"/edit' class='link'>メッセージ</a></div><br class='d-md-none'></div>"; 
                }
            }, 
            { data: 'is_active', 
                name:'status',
...
/***********coustom search input start ************/
    $('#page_search').on( 'click', function () {
        ...
        table.page.len(per_page).ajax.url(SITEURL+'/buyer/list?is_active='+status+'&start_date='+start_date+'&end_date='+end_date).load();  
    } );
...
var deleteUrl = SITEURL + '/buyer/delete'; 
var statusUrl = SITEURL + '/buyer/status-change';
var user_check_id = [];
var opt_group =[];
