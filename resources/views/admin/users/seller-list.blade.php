    ...
    <section id="scrollTop" class="box-content">       
        ...
        {!! Form::open(array('class'=> 'form-block', 'method'=>'POST', 'enctype' => 'multipart/form-data','id'=> 'filterForm')) !!}
            ...
            <div class="form-content">
                ...
                <div class="form-group d-flex">
                    <div class="label-section d-flex align-items-center">
                        {!! Form::label('vendor', '地域商社', ['class' => 'col-form-label']) !!}
                    </div>
                    <div class="field-section">
                        {!! Form::select('users', $users, null, ['placeholder' => 'すべて', 'class' => 'custom-select users select2','id' => 'search_localvendor']) !!}
                    </div>
                </div>
                <div class="form-group d-flex">
                    <div class="label-section d-flex align-items-center">
        ...
        columns: [
            ...
            },
            {
                ...
                'render': function (data,type,row) { 
                    return "<div class = 'vendor-first'><span style='display:block; margin-bottom:-11px;'><a href='/users/"+row.id+"/edit'class='link'>"+data+"</a></span><div class = 'pr-sm-0 pr-md-5' style = 'display:inline-block;'><span><a href='/products/"+row.id+"/edit'><img class='icon-support pr-2' style = 'height:25px;' src='{{ asset('image/icon-product-gray.png') }}' alt='island icon'></a></span><a href='/products/"+row.id+"/edit' class='link'>商品一覧</a></div><br class='d-md-none'><div class = 'pr-sm-0 pr-md-5 pt-sm-3'  style = display:inline-block;'><span><a href='/products/read/"+row.id+"'><img class='icon-support pr-2' style = 'height:25px' src='{{ asset('image/message.png') }}'' alt='island icon'></a></span><a href='/products/read/"+row.id+"' class='link'>メッセージ</a></div><br class='d-md-none'><div class = 'pt-sm-3 pt-md-3' style = 'display:inline-block;'><span><a href='/seller/profile/"+row.id+"/edit' class='circle'><i class='fa fa-user-circle-o fa-2x pr-2'></i></a></span><a href='/seller/profile/"+row.id+"/edit' class='link'>プロフィール編集</a></div></div>"; 
                }
            }, 
            { data: 'vendor', 
                name: 'vendor', 
...
/***********coustom search input start ************/
    $('#page_search').on( 'click', function () {
        ...
        var localvendor = $("#search_localvendor").val();
        ...
        table.page.len(per_page).ajax.url(SITEURL+'/operator/top?is_active='+status+'&search_keword='+keyword+'&start_date='+start_date+'&end_date='+end_date+'&search_rank='+rank+'&search_island='+island+'&search_localvendor='+localvendor+'&search_category='+category+'&search_seller='+seller+'&salesCheckbox='+salesCheckbox).load();  
    } );
