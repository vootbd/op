...
@push('custom-style')
<link rel="stylesheet" href="{{ asset('css/admin/css/vendor-list.css') }}">
...
<li class="breadcrumb-item active" aria-current="page">地域商社一覧</li>
@endpush
        ...
        columns: [
            ...
            },
            {
                ...
                'render': function (data,type,row) {
                    return "<div class = 'vendor-first'><span style='display:block; margin-bottom:-11px;'><a href='/users/"+row.id+"/edit' class='link'>"+data+"</a></span><div class = 'pr-sm-0 pr-md-5' style = 'display:inline-block;'><span><a href='/products/"+row.id+"/edit'><img class='icon-support pr-2' style = 'height:25px; display:inline-block' src='{{ asset('image/icon-product-gray.png') }}' alt='island icon'></a></span><a href='/products/"+row.id+"/edit' class='link' style='display:inline-block'>商品一覧</a></div><br class='d-md-none'><div class = 'pr-sm-0 pr-md-5 pt-sm-3'  style = display:inline-block;'><span><a href='/products/read/"+row.id+"'><img class='icon-support pr-2' style = 'height:25px' src='{{ asset('image/message.png') }}'' alt='island icon'></a></span><a href='/products/read/"+row.id+"' class='link'>メッセージ</a></div><br class='d-md-none'><div class = 'pt-sm-3 pt-md-3' style = 'display:inline-block;'><span><a href='/users/"+row.id+"/edit' class='circle'><i class='fa fa-user-circle-o fa-2x pr-2'></i></a></span><a href='/users/"+row.id+"/edit' class='link'>プロフィール編集</a></div></div>";
                }
            },
            { data: 'is_active',
                name:'status',
...
/***********coustom search input start ************/
    $('#page_search').on( 'click', function () {
        ...
        table.page.len(per_page).ajax.url(SITEURL+'/localvendor/list?is_active='+status+'&search_keword='+keyword+'&start_date='+start_date+'&end_date='+end_date+'&search_island='+island+'&search_category='+category+'&search_seller='+seller+'&salesCheckbox='+salesCheckbox).load();
    } );
...
var deleteUrl = SITEURL + '/localvendor/delete';
var copyUrl = SITEURL + '/localvendor/copy';
var statusUrl = SITEURL + '/localvendor/list/status-change';
var user_check_id = [];
var opt_group =[];
