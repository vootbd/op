...
@push('custom-style')
<link rel="stylesheet" href="{{ asset('css/admin/css/localvendor-product-list.css') }}">
...
<div class="inner-content">
   ...
   @endif
    ...
    <section id="scrollTop" class="box-content">
        ...
        {!! Form::open(array('route' => ['csv.product.export'],'class'=> 'form-block', 'method'=>'POST', 'enctype' => 'multipart/form-data','id'=> 'filterForm')) !!}
            @csrf
...
/************* Drop Down Dependency start ****************/
    jQuery('select[name="island"]').on('change',function(){
            ...
            {
                jQuery.ajax({
                    url : '/dropdown/getseller-island/'+islandID,
                    type : "GET",
                    dataType : "json",
...
/************* User index page data showing start ****************/
    ...
    table = $('#data-table').DataTable( {
        ...
        serverSide: true,
        ajax: {
            url: SITEURL + '/localvendor/products/list',
            type: 'GET',
            statusCode: {
                401: function(){
                    location.reload()
        ...
        columns: [
            ...
            { data: 'sell_price', name: 'sell_price','width': '13%' },
            {
                data: 'cover_image_md',
                'className': 'dt-body-center',
                'width': '20%',
            ...
            { data: 'seller', name: 'seller','width': '15%'},
            { data: 'status',
                name:'status',
                'width': '10%',
                'render': function (data,type,row) {
...
/***********coustom search input start ************/
    $('#page_search').on( 'click', function () {
        ...
        table.page.len(per_page).ajax.url(SITEURL+'/localvendor/products/list?is_active='+status+'&search_keword='+keyword+'&start_date='+start_date+'&end_date='+end_date+'&search_island='+island+'&search_category='+category+'&search_seller='+seller+'&salesCheckbox='+salesCheckbox).load();
    } );
...
var deleteUrl = SITEURL + '/localvendor/products/remove';
var copyUrl = SITEURL + '/products/localvendor/copy';
var statusUrl = SITEURL + '/products/localvendor/status-change';
var user_check_id = [];
var opt_group =[];
